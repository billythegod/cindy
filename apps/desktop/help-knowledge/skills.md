...
- Uploads use Cindy's signed-in account. The Agent can list your published Skills, upload the folder, and check scan/review status. Upload success does not mean public approval. Updating requires the original author and preserves visibility and ownership. This works in a local desktop task, including when controlled from a phone; SSH files must first be made available on the Cindy host.
