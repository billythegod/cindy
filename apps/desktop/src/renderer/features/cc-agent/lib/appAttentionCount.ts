...
export interface AppAttentionCountInput {
  ...
  getRemoteActivity: (
    sessionId: string,
    deviceId: string,
  ) => SidebarRightStatusInput['liveActivity'];
...
export function countAppAttention(input: AppAttentionCountInput): number {
  ...
  for (const session of input.sessions) {
    ...
    const activity = projectSidebarSessionActivity({
      ...
      liveActivity: session.deviceLinkDeviceId
        ? input.getRemoteActivity(session.id, session.deviceLinkDeviceId)
        : input.localActivities.get(session.id),
