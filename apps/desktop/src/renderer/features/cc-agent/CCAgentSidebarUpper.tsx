...
function ExpandedView({
  ...
  // Keep the include-pinned catalogue available to both filtered consumers.
  ...
  const pruneSelectionToRenderedRows = useCallback(() => {
    ...
    if (selectionAnchorSessionIdRef.current && !renderedSessionIds.has(selectionAnchorSessionIdRef.current)) {
  ...
  const handleSessionClick = useCallback(
    async (id: string, modifiers?: SessionClickModifiers) => {
      ...
      // Cached ordinary/lead tasks need no lookup. Keep navigation in the
      // click batch instead of flushing sidebar updates across an await.
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      if (selectionAnchorSessionIdRef.current && succeededIds.has(selectionAnchorSessionIdRef.current)) {
        selectionAnchorSessionIdRef.current = null;
      }
