...
function OrcaCollabCard({ card, screenWidth, blockKey }: {
  ...
  // This card defaults open. Remember the user's collapse choice using the
  // existing bounded block store, including across native route reconstruction.
  const [collapsed, toggleCollapsed] = useFoldableExpandedState(
    `orca-collapsed-${JSON.stringify([accountGeneration, blockKey])}`, false,
  );
  const { colors } = useTheme();
  const styles = useThemedStyles(makeStyles);
  ...
  return (
    <CollabCardShell
      leadingIcon={<Bot color={colors.textTertiary} size={iconSize.md} strokeWidth={iconStroke.regular} />}
      title={card.title}
      controlledExpanded={!collapsed}
      onControlledToggle={toggleCollapsed}
