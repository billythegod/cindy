...
describe('quick-switch panel sizing', () => {
  it.each([280, 320, 393, 600, 744, 852, 1366])('offers switching at width %i without replacing back', (windowWidth) => {
  ...
  it('keeps the existing tablet sizing and fits narrow split windows', () => {
    ...
    expect(buildWideSessionNavLayout({ windowWidth: 280 }).drawerWidth).toBe(256);
  });
  it.each([undefined, Number.NaN, -1, 0])('does not open before a valid layout (%s)', (windowWidth) => {
