...
describe('mobile session header desktop-first surface', () => {
  ...
  it('keeps the visible header chrome compact while preserving full settings access', () => {
    ...
    expect(source).toContain('<View style={[styles.sessionChromeContent, { paddingTop: horizontalSystemHeader ? nativeHeaderHeight : insets.top + (paneLayout.persistent ? spacing.lg : 0) }]}>');
