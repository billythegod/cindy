...
export function registerScheduleCreateTool(
  ...
  registry.register({
    ...
    description:
      ...
      '(7) 新建检查默认 silentWhenIdle=true（无变化不播报）；明确提醒/定时发送设 false，保留用户选择。如果用户定义了提醒条件，把条件保留在 prompt 里（如 CI 失败/新评论时提醒），有结果时用 schedule_notify_current_run 主动上报；' +
    ...
    inputShape: {
      ...
      silentWhenIdle: z
        .boolean()
        .optional()
        .describe('静默运行:true → 成功 run 默认不通知、不产生未读红点;任务 prompt 可自行说明哪些条件需要提醒,agent 满足条件时调用 schedule_notify_current_run 主动上报。失败轮仍会通知。新建 agent 自动化默认 true；明确提醒/定时发送设 false，不吞用户要求的发送。'),
      preRunHook: z
        .object({
          command: z
            .string()
