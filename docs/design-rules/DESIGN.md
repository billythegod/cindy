...
Desktop task errors do not display reminder dots in task rows, pinned cards, or project/device aggregates. Keep running, awaiting-input, and successful-unread indicators. Errors and interrupted runs remain available inside the task; automatic-run failure notices link to the corresponding automation with “View Details”. This does not change native Agent Island, mobile, or OS notification behavior.
